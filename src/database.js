        ...
        db.run(`CREATE TABLE IF NOT EXISTS activiteit (
            ...
            db.all(`SELECT COUNT() AS aantal FROM activiteit`, (err, rows) => {
                if (rows[0].aantal == 0) {
                    ...
                    db.run(insert, ['Speel uitdagertje', 'Maak twee ploegen en zet iedere ploeg in een lijn tegenover elkaar.\nBij de ene ploeg steekt iedereen de handen uit. Van de andere ploeg komt iemand af en tikt om beurten bij een speler bovenaan de handen. Als hij onderaan op de handen tikt moet de persoon bij wie er onderaan getikt werd, deze persoon proberen tikken voordat die terug bij zijn ploeg is', 'overal', 'altijd', '6', '8', '']);
